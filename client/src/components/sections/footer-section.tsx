...
const Footer = () => {
  ...
  return (
    <footer className={`py-12 px-4 sm:px-6 lg:px-8 ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-800'}`}>
      <div className="container mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        <div className="col-span-1 sm:col-span-2 lg:col-span-1">
          ...
          <p className={`text-sm lg:text-base font-normal mb-4 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
          ...
          <p className={`text-sm lg:text-base font-normal mb-4 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
            <a
              ...
              className={`hover:text-yellow-600 transition-colors ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}
            >
              (+800) 1234 5678 90
            </a>
            {" - "}
      ...
      <div className={`mt-8 pt-8 border-t ${theme === 'dark' ? 'border-gray-700 text-gray-400' : 'border-gray-300 text-gray-600'} text-center text-sm`}>
        <p className="mb-2">
...
const FooterColumn: React.FC<FooterColumnProps> = ({ heading, links }) => {
  ...
  return (
    <div className="col-span-1">
      <h3 className={`text-lg font-medium mb-4 ${theme === 'dark' ? 'text-gray-200' : 'text-gray-700'}`}>{heading}</h3>
      <ul className="space-y-2">
        {links.map((link, index) => (
          <li key={index}>
            <Link
              to={link.href}
              className={`text-sm lg:text-base hover:text-yellow-600 transition-colors ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}
            >
              {link.name}
            </Link>
          </li>
