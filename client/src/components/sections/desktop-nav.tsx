...
const members: { title: string; href: string; description: string }[] = [
...

  {
    ...
    description: "An online platform for buying, selling, or trading goods and services.",
  },
];
...
export function NavItems({ userId, registrationFee, userRole, membershipStatus, onboardingStatus }: NavItemsProps) {

  const isAdmin = !(userId !== undefined && registrationFee && membershipStatus === "ACTIVE" && userRole === "ADMIN" && onboardingStatus);
  const isMember = !(userId !== undefined && registrationFee && membershipStatus === "ACTIVE" && onboardingStatus);
        ...
        <NavigationMenuItem>
          ...
          <NavigationMenuContent>
            <ul className="grid gap-3 p-6 md:w-[400px] lg:w-[500px] lg:grid-cols-[.75fr_1fr]">
              <li className="row-span-3">
                <NavigationMenuLink asChild>
                  <Link to="/" className="flex h-full w-full select-none flex-col justify-end rounded-md bg-gradient-to-b from-muted/50 to-muted p-6 no-underline outline-none focus:shadow-md">
                    <img src="/assets/images/NdiIgbo_Sunderland_transparent.png" width={128} height={38} alt="Ndiigbo logo" />
                    <div className="mb-2 mt-4 text-lg font-medium">Ndi Igbo Sunderland</div>
                    <p className="text-sm leading-tight text-muted-foreground">
                      A vibrant and dynamic Nigerian community in Sunderland, fostering cultural heritage, unity, and social engagement among members.
                    </p>

                  </Link>
                </NavigationMenuLink>
              </li>
              <ListItem id="about-us" href="/about-us" title="About Ndi-Igbo Sunderland">
                A vibrant Nigerian community promoting culture and unity in Sunderland.
              </ListItem>
              ...
              {!isAdmin ? (
                <ListItem href="/admin-screens/board-routes" title="Manage Members">
            ...
            <NavigationMenuItem>
              ...
              <NavigationMenuContent>
                <ul className="grid w-[400px] gap-3 p-4 md:w-[500px] md:grid-cols-2 lg:w-[600px] ">
                    ...
                    {members.map((member) => (
                      <ListItem className="pointer-events-none text-gray-500" key={member.title} title={member.title} href={member.href}>
                        {"Sorry! members only..."}
                      </ListItem>
                    ))}
                </ul>
