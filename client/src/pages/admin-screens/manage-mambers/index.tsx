...
function ManageMembersPage() {
  ...
  const handleEdit = (id: string) => {
    const member = membersData?.find((member: GetMembersResponseType[number]) => member.id === id);
    if (member) {
  ...
  const transformedMembersData = membersData.map((member: GetMembersResponseType[number]) => ({
    "id": member.id,
    "first_name": member.first_name,
    "last_name": member.last_name,
    "email": member.email,
    "birth_date": member.birth_date ? new Date(member.birth_date).toISOString() : null,
