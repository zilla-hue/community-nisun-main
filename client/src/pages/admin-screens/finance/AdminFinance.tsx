...
import { fetchTransactions, getStoredTransactions } from "../../../api/transactionApi";
...
export default function AdminFinance() {
  ...
  useEffect(() => {
    if (transactions.length > 0) {
      ...
      const calculateData = (accountType: string) => {
        ...
        const accountTransactions = accountType === "all"
          ? transactions
        ...
        const currentPeriodTransactions = accountTransactions.filter((transaction) => {
          const transactionDate = parseISO(transaction.date);
        ...
        const previousPeriodTransactions = accountTransactions.filter((transaction) => {
          const transactionDate = parseISO(transaction.date);
          return isWithinInterval(transactionDate, {
        ...
        const remainingChange = previousRemaining === 0 
          ? 100 // If previous remaining was 0, treat as 100% increase
          : ((currentRemaining - previousRemaining) / Math.abs(previousRemaining)) * 100;
        ...
        console.log('Period Data:', {
          currentPeriod: { income: currentPeriodIncome, expenses: currentPeriodExpenses },
          previousPeriod: { income: previousPeriodIncome, expenses: previousPeriodExpenses },
          changes: { income: incomeChange, expenses: expensesChange, remaining: remainingChange }
        });
  ...
  useEffect(() => {
    ...
    const handleTransactionsUpdated = (event: CustomEvent<{ transactions: Transaction[] }>) => {
      console.log('AdminFinance received transaction update:', event.detail.transactions);
            ...
            <Card className={`shadow-lg hover:shadow-xl transition-shadow ${
              ...
              <CardContent>
                ...
                >
                  ...
                  {Math.abs(currentData.remainingChange)}% {currentData.remainingChange >= 0 ? 'increase' : 'decrease'} from last period
                </p>
              </CardContent>
            </Card>
            ...
            <Card className={`shadow-lg hover:shadow-xl transition-shadow ${
              ...
              <CardContent>
                ...
                <p className="text-sm flex items-center mt-1 text-green-500">
                  <ArrowUpIcon className="w-4 h-4 mr-1" />
                  {Math.abs(currentData.incomeChange)}% increase from last period
                </p>
              </CardContent>
            </Card>
            ...
            <Card className={`shadow-lg hover:shadow-xl transition-shadow ${
              ...
              <CardContent>
                ...
                <p className="text-sm flex items-center mt-1 text-red-500">
                  <ArrowDownIcon className="w-4 h-4 mr-1" />
                  {Math.abs(currentData.expensesChange)}% decrease from last period
                </p>
              </CardContent>
            </Card>
          </div>
