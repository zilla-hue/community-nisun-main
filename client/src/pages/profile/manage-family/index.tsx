...
function Page() {
  ...
  const { mutate: addChild, isPending: isAddingChild } = useMutation({
    ...
    mutationFn: async () => addChildAction({
      first_name,
      last_name,
  ...
  const { data: childrenData, error, isLoading } = useQuery<GetChildrenResponseType>({
    queryKey: childrenQueryKey,
    queryFn: () => fetch(`/api/children`).then((res) => {
      if (!res.ok) {
  ...
  const isTableDisabled = isLoading || isDeletingChildren || isDeletingChild || isAddingChild || isEditingChild;

  if (isLoading) return <div className="w-full pb-10 mt-4">
          ...
          <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
            ...
            <SheetContent>
              <SheetHeader>
                ...
                <SheetDescription>
                  {isEdit ? "Update the details of the child." : "Register a child. Note that the family member must be below 18 years old."}
                </SheetDescription>
              </SheetHeader>
              <form onSubmit={handleSubmit}>
                ...
                <SheetFooter>
                  <Button type="submit" disabled={isAddingChild || isEditingChild}>
                    {isEdit ? (isEditingChild ? "Updating..." : "Update Child") : (isAddingChild ? "Submitting..." : "Save Changes")}
                  </Button>
                </SheetFooter>
              </form>
            </SheetContent>
