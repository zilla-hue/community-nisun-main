  ...
  return (
    <>
      <div className="flex justify-end">
        <h1 className="mt-4 mb-2 items-center font-semibold text-sm">REVIEW DETAILS</h1>
      </div>
              ...
              <AccordionItem value="item-1">
                ...
                <AccordionContent>
                  {Object.entries(formData)
                    .filter(([key]) => !["nok_"].some((prefix) => key.startsWith(prefix)))
                    .map(([key, value]) => (
                  ...
                  {Object.entries(formData)
                    ...
                    .map(([key, value]) => (
                      <div className="grid grid-cols-2 gap-x-4 py-2" key={key}>
                        <label className="font-semibold">{key.replace("nok_", "").replace("_", " ")}</label>
                        <p className="ml-2">{value}</p>
                      </div>
                    ))}
                </AccordionContent>
